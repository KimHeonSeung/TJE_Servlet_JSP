...
<script type="text/javascript">
	...
	// tip. id 중복체크 한 뒤 다시 인풋텍스트를 사용자가 수정했을 시 다시 중복확인을 하게 하는 방법 
	// - focus out을 할 때 DB를 다녀온다.
	// - 최종적으로 가입하기를 눌렀을때 다시 DB를 갓다온다.
...
<form id="user_regist_form" style="margin-right:auto;margin-left:auto;" action="<%= request.getContextPath() %>/regist.khs" method="post">
	<table border="1" style="width:600px; height:500px">
		<tr>
			<th>ID(*)</th>
			<td><input class="input-group-text" style="width:500px" type="text" name="user_id" placeholder="1~10자 문자를 입력하세요. (필수입력, 중복체크 필수)" required> 
		...
		<tr>
			<th>PW(*)</th>
			<td><input class="input-group-text" style="width:500px" type="password" name="user_pw" placeholder="1~15자 문자를 입력하세요. (필수입력사항)" required>
			<span id="checkPWArea"></span></td>
		</tr>
		<tr>
			<th>PW 확인(*)</th>
			<td><input class="input-group-text" style="width:500px" type="password" name="user_pw_confirm" required>
			<span id="checkPWConfirmArea"></span></td>
		</tr>
		<tr>
			<th>닉네임(*)</th>
			<td><input class="input-group-text" style="width:500px" type="text" name="user_nick" value="${ param.user_nick }" placeholder="1~8자 문자를 입력하세요. (필수입력, 중복체크 필수)" required > 
		...
		<tr>
			<th>연락처</th>
			<td><input class="input-group-text" style="width:500px" type="number" name="user_tel" value="${ param.tel }" placeholder="1~11자 숫자를 입력하세요. (선택입력사항)" >
			<span id="checkTelArea"></span>
			</td>
		</tr>
		<tr>
			<th>이메일</th>	
			<td><input class="input-group-text" style="width:500px" type="text" name="user_mail" value="${ param.address }" placeholder="1~30자 문자를 입력하세요. (선택입력사항)">
			<span id="checkMailArea"></span>
			</td>		
		</tr>		
		<tr>
			<th colspan="2"><input class="btn btn-primary" type="submit" name="submit" value="가입하기">
			<input type="reset" class="btn btn-primary" value="초기화"></th>
		</tr>
	</table>
