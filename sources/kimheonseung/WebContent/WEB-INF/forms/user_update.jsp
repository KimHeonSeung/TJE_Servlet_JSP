...
<script type="text/javascript">
	...
	$(function () {
		// 닉네임 중복체크를 안했을 시
		if($("#user_regist_form [name='user_nick']").val() == '${login_user.user_nick}'){
			isCheckNick = true;
		}
...
<form id="user_regist_form"  action="<%= request.getContextPath() %>/update_result.khs" method="post">
	<table border="1" style="width:600px; height:500px">
		<tr>
			<th>ID(*)</th>
			<td><input class="input-group-text" type="text" style="width:500px" name="user_id" value="${ login_user.user_id }" readonly> 
			</td>
		</tr>
		<tr>
			<th>PW(*)</th>
		...
		<tr>
			<th>닉네임(*)</th>
			<td><input class="input-group-text" style="width:500px" type="text" name="user_nick" value="${ login_user.user_nick }" required > 
		...
		<tr>
			...
			<c:if test="${ login_user.user_tel eq -1 }" var="no">
				<td><input class="input-group-text" style="width:500px" type="number" name="user_tel">
				<span id="checkTelArea"></span>
				</td>
			</c:if>
			<c:if test="${ not no }">
				<td><input class="input-group-text" style="width:500px" type="number" name="user_tel" value="${ login_user.user_tel }">
				<span id="checkTelArea"></span>
				</td>
			</c:if>
		...
		<tr>
			...
			<c:if test="${ empty login_user.user_mail }" var="no">
				<td><input class="input-group-text" style="width:500px" type="text" name="user_mail">
				<span id="checkMailArea"></span>
				</td>		
			</c:if>
			<c:if test="${ not no }">
				<td><input class="input-group-text" style="width:500px" type="text" name="user_mail" value="${ login_user.user_mail }" >
				<span id="checkMailArea"></span>
				</td>		
			</c:if>
		</tr>		
