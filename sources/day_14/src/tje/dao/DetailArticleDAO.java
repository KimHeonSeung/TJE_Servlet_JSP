...
public class DetailArticleDAO {
	...
	public boolean update(Connection conn, DetailArticle obj) {
		boolean result = false;
		String sql = "update article set title=?, content=?, last_update_time=now() where article_id=?";
		// article_id, member_id, title, content, write_time, last_update_time, read_count !!
		PreparedStatement pstmt = null;		
		...
		try {
			...
			pstmt.setString(2, obj.getContent().replaceAll("\n", "<br>"));			
